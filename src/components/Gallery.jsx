...
const neonColors = ["#00FFC2", "#FF00FF", "#00C2FF", "#FFFF00"]
...
const GlitchText = ({ children, isDark, primaryColor, className = "" }) => {
  ...
  const glitchVariants = {
    ...
    hover: {
      opacity: [1, 0.8, 0.1, 0.8, 1],
      x: [0, 2, -2, 0],
      transition: {
        duration: 0.2,
        ease: "easeInOut",
        times: [0, 0.2, 0.4, 0.6, 1],
        onComplete: () => setFlickerColor(getRandomNeonColor()),
      },
    },
    idleFlicker: {
      opacity: [1, 0.9, 1],
...
const GlitchGalleryItem = ({ item, index, isDark }) => {
  ...
  return (
    ...
    >
      ...
      <motion.div className="relative w-full h-full" variants={glitchImageVariants} whileHover="hover">
        <img
...
export default function Gallery() {
  ...
  const bgLayer3Y = useTransform(scrollYProgress, [0, 1], ["0%", "80%"])
  ...
  return (
    ...
    >
      {isDark && (
        <>
          ...
          <motion.div
            ...
            style={{
              backgroundImage: `url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="100" height="100" fill="none" stroke="${primaryColor.replace("#", "%23")}30" strokeWidth="0.5"><path d="M50 0L100 50L50 100L0 50L50 0Z" /><circle cx="50" cy="50" r="25" /></svg>')`,
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div ref={containerRef} className="relative grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 pt-20">
          {gallerySample.map((item, index) => (
            <GlitchGalleryItem key={item.id} item={item} index={index} isDark={isDark} />
          ))}
        </div>
